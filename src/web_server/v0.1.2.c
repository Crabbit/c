/*-----------------------------------------------------------------------------------
  This software is copyright in 2012.11.19 by Lili.
...
void Open_Html( int connect_fd,char Read_Buf[] , char path[]);
...
//receive the information of the clients request
	...
	Open_Html( connect_fd,Read_Buf,path );
...
//get the length of the index.html,and keep the point is in the start
	...
	sendfile( connect_fd,FileD, path, len );

//	if( ( ret = read( FileD, Read_Buf, len ) ) < 0 )	
//		my_err( "Read",__LINE__ );

//	if( Read_Buf =( char * ) mmap( 0, len, PROT_READ, MAP_PRIVATE, FileD, 0 ) == MAP_FAILED )
//		my_err( "Mmap",__LINE__ );
*/

	if( fstat ( FileD, &buf) == -1 )
		my_err( "Can't stat the file",__LINE__ );

	printf( "The file size is %u bytes\n",buf.st_size );

	while( off < buf.st_size )
	{
		if( ( res = sendfile( connect_fd, FileD, &off,buf.st_size ) ) < 0)
			my_err( "Send File failed",__LINE__ );
	}
