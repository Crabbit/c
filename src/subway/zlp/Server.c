...
//status < 0.----means transport has problem
		...
		if( status >= 0 )
		{
			...
			v0 = v1 = -1;
			readPlatformName(&v0, start_platform), readPlatformName(&v1, end_platform);
			if(!strncmp("-1", start_platform, 2) || !strncmp("-1", end_platform, 2)){

				strcpy(send_buf, "error没有找到匹配的站名#");
				send( connect_fd, send_buf, strlen( send_buf ), 0 );
...
//			resultInfo2;
				...
				printf( "ResultInfo1 : ----  %s\n",resultInfo1 );			
				printf( "ResultInfo2 : ----  %s\n",resultInfo2 );			

				for(i=0 ;strncmp("-1", resultInfo1[i], 2); i++){
				
					strcat(send_buf, resultInfo1[i]);
					strcat(send_buf, "#");
				}
				strcat(send_buf, "$");
				for(i=0 ;strncmp("-1", resultInfo2[i], 2); i++){
				
					strcat(send_buf, resultInfo2[i]);
					strcat(send_buf, "#");
				}
				
	//			printf( "Xun huan 1 OK \n" );
				strcat(send_buf, "$");

				printf("%s\nprintf : ResultInfo1 + resultInfo2\n",send_buf);
			...
			else
			{
				...
				runRideShort(G, v0, v1);
//				resultInfo2;

				printf( "ResultInfo2 : ----  %s\n",resultInfo2 );			

				send_buf[0] = 0;
				if(!strncmp("-3", resultInfo2[0], 2))
					strcpy(send_buf, resultInfo2[1]);
				else{
